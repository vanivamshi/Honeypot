...
def connect_to_server(host="127.0.0.1", port=8080, username="admin", password="password123"):
    ...
    try:
        ...
        post_data = "username={username}&password={password}".format(username=username, password=password)
        request = (
            "POST /login HTTP/1.1\r\n"
            "Host: {host}:{port}\r\n"
        ...
        if client:
            ...
            logging.info("{timestamp} - Connection closed".format(timestamp=timestamp))
